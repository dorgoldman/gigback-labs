...
**Status (2026-07-13): Partially complete.** 1.1 (Perplexity research), 1.5 (validation memo), 1.6 (content map) done — enough to greenlight Phase 3. **1.2 (Claude in Chrome field research), 1.3 (keyword/PAA mining), 1.4 (personas) were skipped**, not folded into 1.1. No primary-source field data or documented personas exist yet. Worth doing before picking the next content batch (topics 7–12).
...
**Status (2026-07-13):** Done out of order — 2.3 shipped ahead of Phase 3 (needed the guide template to publish guides). **2.1 and 2.2 done**: formal review run, findings saved to `docs/reports/uiux-review-1.md`, and the actionable fixes implemented same-session (WCAG AA contrast on text-muted/text-subtle/links, 44×44px touch targets on hamburger/lang-toggle/notify-links, unified focus ring, honeypot aria-hidden). **2.5 done**: new `favicon.svg` (single "G" mark, replaces the cramped "GL"), wired into all 11 pages at the correct relative path, verified 200 in the browser. **2.4 is 🟡 code-complete, one manual step pending**: `og.svg` built (1200×630, on-brand, Hebrew headline, verified no overlaps/out-of-bounds), and every page's `<meta property="og:image">` already points to `/og.png` — but social platforms need a raster file and no SVG→PNG converter exists in this environment. Conversion instructions are in `docs/manual-tasks.txt` (item 4), ~2 minutes with a free online tool.
